...
> 2026-05-01 更新：配置在 dummy 网卡上的 IP 段 v4:`/27` 和 v6:`/48` 分别改为 `/32` 和 `/128`
> 这包括 `/etc/netplan/60-dn42-dummy.yaml` 中的配置，以及文中示例命令 `ip addr add dev dn42-dummy <路由器IP>/<子网>` 中的 `<子网>` 部分。
...
> 改为 fd24:ac8e:9e04::3/128 后，direct protocol 产生的是主机路由，每个节点通过 Babel/OSPF 等协议，只通告自己的 loopback 地址，对端就能学到一条指向隧道的具体路由。
